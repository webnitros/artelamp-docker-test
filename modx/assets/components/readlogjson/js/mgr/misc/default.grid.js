ReadLogJson.grid.Default = function (config) {
    config = config || {};
    ...
    Ext.applyIf(config, {
        ...
        baseParams: {},
        cls: config['cls'] || 'main-wrapper readlogjson-grid',
        autoHeight: true,
        paging: true,
        remoteSort: true,
        ...
        viewConfig: {
            ...
            getRowClass: function (rec) {
                ...
                if (rec.data['published'] != undefined && rec.data['published'] == 0) {
                    cls.push('readlogjson-row-unpublished');
                }
                if (rec.data['active'] != undefined && rec.data['active'] == 0) {
                    cls.push('readlogjson-row-inactive');
                }
                if (rec.data['deleted'] != undefined && rec.data['deleted'] == 1) {
                    cls.push('readlogjson-row-deleted');
                }
                if (rec.data['required'] != undefined && rec.data['required'] == 1) {
                    cls.push('readlogjson-row-required');
                }
                return cls.join(' ');
            }
        },
    });
    ReadLogJson.grid.Default.superclass.constructor.call(this, config);
...
Ext.extend(ReadLogJson.grid.Default, MODx.grid.Grid, {
    ...
    getSearchField: function (width) {
        return {
            xtype: 'readlogjson-field-search',
            width: width || 250,
            listeners: {
                search: {
                    fn: function (field) {
    ...
    getMenu: function (grid, rowIndex) {
        ...
        var menu = ReadLogJson.utils.getMenu(row.data['actions'], this, ids);
...
Ext.reg('readlogjson-grid-default', ReadLogJson.grid.Default);
