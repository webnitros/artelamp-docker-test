...
const elemTemplate0 = new Ext.XTemplate('<tpl for=".">\
						<div class="x-combo-list-item">\
							<tpl if="id">({id})</tpl>\
							<strong>{name}</strong><small>({category_name})</small>\
						</div>\
					</tpl>', {compiled: true})
const elemTemplate2 = new Ext.XTemplate('<tpl for=".">\
						<div class="x-combo-list-item">\
							<tpl if="id">({id})</tpl>\
							<strong>{templatename}</strong><small>({category_name})</small>\
						</div>\
					</tpl>', {compiled: true})
const elemTemplate3 = new Ext.XTemplate('<tpl for=".">\
						<div class="x-combo-list-item">\
							<strong>{text_lex}</strong><small>({parent}),({namespace})</small>\
						</div>\
					</tpl>', {compiled: true})
const elemTemplate4 = new Ext.XTemplate('<tpl for=".">\
						<div class="x-combo-list-item">\
							<strong>{name_trans}</strong><small>({namespace})</small>\
						</div>\
					</tpl>', {compiled: true})
const elemTemplate5 = new Ext.XTemplate('<tpl for=".">\
						<div class="x-combo-list-item">\
							<strong>{url}</strong><small>({snippet})</small>\
						</div>\
					</tpl>', {compiled: true})
const elemTemplate6 = new Ext.XTemplate('<tpl for=".">\
						<div class="x-combo-list-item">\
...
const todata = function(str, key = 'name') {
	if(str) {
...
EasyPack.window.add = function(config) {
	...
	var fields = [
		...
		},
		{
			...
			url: easypackConnectorUrl,
			baseParams: {
				action: 'show/getmenu', combo: 1, sort: 'namespace',
				dir: 'DESK',
			},
			allowBlank: true,
			valueField: 'text',
