msPromoCode2.grid.Default = function (config) {
    config = config || {};

    if (typeof(config['multi_select']) !== 'undefined' && config['multi_select'] == true) {
        config['sm'] = new Ext.grid.CheckboxSelectionModel();
    }
    config['cls'] = (config['cls'] || 'main-wrapper') + ' mspc2-grid';
    ...
    Ext.applyIf(config, {
        ...
        viewConfig: {
            ...
            getRowClass: function (rec) {
                var cls = ['mspc2-grid-row'];
                if (rec.data['active'] != undefined && rec.data['active'] == 0) {
                    cls.push('mspc2-grid-row_disabled');
                }
                return cls.join(' ');
            },
        },
    ...
    msPromoCode2.grid.Default.superclass.constructor.call(this, config);
...
Ext.extend(msPromoCode2.grid.Default, MODx.grid.Grid, {
    ...
    getMenu: function (grid, rowIndex) {
        ...
        var menu = msPromoCode2.utils.getMenu(row.data['actions'], this, ids);
    ...
    _doAction: function (action, action_prefix, confirm, check_id) {
        ...
        if (confirm) {
            MODx.msg.confirm(Ext.apply({}, params, {
                title: ids.length > 1
                    ? _('mspc2_button_' + action + '_multiple')
                    : _('mspc2_button_' + action),
                text: _('mspc2_confirm_' + action),
            }));
        } else {
...
Ext.reg('mspc2-grid-default', msPromoCode2.grid.Default);