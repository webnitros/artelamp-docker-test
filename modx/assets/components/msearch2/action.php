...
if (!empty($_REQUEST['pageId']) && !empty($_REQUEST['key'])) {
    ...
    $arrays = [
        'q' => $modx->resource->get('uri')
    ];
    $cache_prefix = 'msearch2/settings/' . md5(json_encode($arrays));
    #$config = $modx->cacheManager->get($cache_prefix . '/config');
    $config = $modx->cacheManager->get($cache_prefix . '/properties');


    if (!empty($config['paginatorProperties']['resources'])) {
        #unset($_SESSION['mSearch2'][$_REQUEST['key']]['paginatorProperties']['resources']);
    }
}
...
if (empty($config) || !is_array($config)) {
    if ($action !== 'search') {
        ...
        // Доработка для своего автокомплита
        $config = $scriptProperties = [
            'queryVar' => 'query',
            'autocomplete' => TRUE,
            'limit' => 5,
    ...
    case 'search':
            ...
            switch (strtolower($scriptProperties['autocomplete'])) {
                ...
                default:
                    ...
                    if (!empty($found)) {
                        ...
                        $ids = $pdoFetch->runSnippet($snippet, $scriptProperties);

                        $processed = ['pagetitle' => TRUE];
                        $criteria = [
                            'resources' => $ids,
                            'where' => [
                                'class_key' => 'msProduct',
                        ...
                        $rows = $pdoFetch->getCollection('modResource', NULL, $criteria);
                        foreach ($rows as $k => $row) {
                            $label = $mSearch2->Highlight($row['pagetitle'], $query);


                            /* @var msProductData $ProductData */
                            if ($ProductData = $modx->getObject('msProductData', $row['id'])) {
                                $row = array_merge($row, $ProductData->get(['price', 'old_price', 'sale', 'thumb']));
                            }

                            $results[] = [
                                'id' => $row['id'],
                                'url' => $modx->makeUrl($row['id'], 'web', '', 'full'),
                                'value' => html_entity_decode($row['pagetitle'], ENT_QUOTES, 'UTF-8'),
                                'label' => !empty($label) ? $label : $row['pagetitle'],
                                'thumb' => $row['thumb'],
                                'price' => $row['price'],
        ...
        $response = [
            ...
            'data' => [
                'results' => $results,
                'total' => count($results),
                'total_all' => !empty($found) ? count($found) : 0,
