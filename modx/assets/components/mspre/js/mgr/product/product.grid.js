mspre.grid.product = function (config) {
    ...
    this.sm = new Ext.grid.CheckboxSelectionModel({
        renderer: function (b, c, a) {return '<div class="x-grid3-row-checker">&#160;</div><div class="x-grid3-row-favorite"></div>'},
...
Ext.extend(mspre.grid.product, MODx.grid.Grid, {
    ...
    getFields: function (config) {
        return mspre.config.fields['select']
    },
    menus: function (name) {
        var actions = {
            menu: {
            ...
            reset: {
                ...
                text: '<i class="icon icon-times"></i> ' + _('mspre_filter_clear'),
                listeners: {
                    click: {
