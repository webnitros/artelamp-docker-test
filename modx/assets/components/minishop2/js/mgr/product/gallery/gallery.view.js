...
Ext.extend(miniShop2.view.Images, MODx.DataView, {
    ...
    downloadMediaServer: function () {
        var product_id = this.config.product_id || '';

        Ext.MessageBox.confirm(
            'Загрузка изображений с медиа севрера',
            'Вы действительно хотите загрузить изображения с медиа сервера?',
            function (val) {
                if (val == 'yes') {
                    this.getEl().mask(_('loading'), 'x-mask-loading');
                    MODx.Ajax.request({
