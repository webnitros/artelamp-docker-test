...
Ext.extend(miniShop2.panel.OrdersForm, MODx.FormPanel, {
	...
	getLeftFields: function(config) {
		return [
			...
			{
				html: 'исключить тестовые заказы',
				xtype: 'combo-boolean',
				id: config.id + '-fake',
				fieldLabel: 'исключить тестовые заказы',
				emptyText: 'исключить тестовые заказы',
				name: 'fake',
				value: false,
				store: new Ext.data.SimpleStore({
					fields: ["d", "v"],
					data: [[_("yes")+', скрывать тестовые', !0], [_("no")+', показывать тестовые', !1]]
				}),
				listeners: {
					select: {
						fn: function() {
