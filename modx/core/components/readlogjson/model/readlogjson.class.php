...
class ReadLogJson
{
    ...
    {
        $this->modx =& $modx;
        $corePath = MODX_CORE_PATH . 'components/readlogjson/';
        $assetsUrl = MODX_ASSETS_URL . 'components/readlogjson/';
        ...
        $this->modx->addPackage('readlogjson', $this->config['modelPath']);
        $this->modx->lexicon->load('readlogjson:default');
        ...
        switch ($ctx) {
            ...
            default:
                if (!defined('MODX_API_MODE') || !MODX_API_MODE) {
                    ...
                    if ($css = $this->modx->getOption('readlogjson_frontend_css')) {
                        $this->modx->regClientCSS(str_replace($config['pl'], $config['vl'], $css));
                    }

                    $config_js = preg_replace(array('/^\n/', '/\t{5}/'), '', '
							ReadLogJson = {};
							ReadLogJsonConfig = ' . $this->modx->toJSON($this->config) . ';
					');


                    $this->modx->regClientStartupScript("<script type=\"text/javascript\">\n" . $config_js . "\n</script>", true);
                    if ($js = trim($this->modx->getOption('readlogjson_frontend_js'))) {
    ...
    public function runProcessor($action = '', $data = array())
    {
        ...
        $processorsPath = !empty($this->config['processorsPath'])
            ...
            : MODX_CORE_PATH . 'components/readlogjson/processors/';
