    ...
    public function submit($data = array())
    {
        ...
        if ($order->save()) {
            ...
            elseif ($payment = $this->modx->getObject('msPayment',
                ...
                // Доработка для отслеживанеи заказов со способами оплаты Онлайн платеж
                if (!$order->get('is_send_admin')) {
                    if (!$order->get('track_order')) {
                        if ($payment->get('track_order')) {
                            $order->set('track_order', true);
                            $order->save();
                        }
                    }
                }
