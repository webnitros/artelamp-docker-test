<?php
	...
	class EasypackBuildCreatFromFolderProcessor extends modProcessor
	{
		...
		public function genPlugins($relativePath, SplFileInfo $element)
		{
			...
			if (!empty($events)) {
				$pp         = trim(trim(str_ireplace(realpath(MODX_CORE_PATH), '', $element->getRealPath()), '\\'), '/');
				$pluginCode = <<<PHP
<?php
	\$path = MODX_CORE_PATH."$pp";
	if(file_exists(\$path)){
		return include \$path;
	}else{
		\$modx->log(MODX_LOG_LEVEL_ERROR, 'Can`t load plugin, file not exist "'.\$path.'"');
	}
PHP;
				$data       = [
					'name'        => $element->name,
