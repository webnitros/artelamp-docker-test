				...
				public function writeTableSchema($schemaFile, $package = '', $baseClass = '', $tablePrefix = '', $restrictPrefix = FALSE, &$classNames)
				{
					...
					foreach ($this->allowed_tables as $table) {
						...
						if ($indexesStmt) {
							...
							if ($this->manager->xpdo->getDebug() === TRUE) {
								$this->manager->xpdo->log(xPDO::LOG_LEVEL_DEBUG, "Indices for table {$table}: " . print_r($indexes, TRUE));
							}
							if (!empty($indexes)) {
								$indices = [];
...
class [+class+] extends [+extends+] {
	...
	public function get(\$name, \$format = NULL, \$formatTemplate = NULL, \$hard = TRUE)
		{
			if (!\$hard AND array_key_exists(\$name, \$this->gets)) {
				return \$this->gets[\$name];
			} else {
				\$v = parent::get(\$name, \$format, \$formatTemplate); // TODO: Change the autogenerated stub
				\$this->gets[\$name] = \$v;
				return \$v;
			}
		}
