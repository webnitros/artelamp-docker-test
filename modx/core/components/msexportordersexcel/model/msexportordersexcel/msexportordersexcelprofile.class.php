...
class msExportOrdersExcelProfile extends xPDOSimpleObject
{
    /* @var array $fields */
    ...
    /* @var msExportOrdersExcelFile $file */
    protected $file = null;
    ...
    public function loadFields($false = false)
    {
        if (is_null($this->fields)) {
            $q = $this->xpdo->newQuery('msExportOrdersExcelProfileFields');
            $q->sortby('rank', 'ASC');
            $q->where(array(
                'active' => 1
            ));
     ...
     * @param msExportOrdersExcelProfileFields $field
     * @return array|boolean
     */
    public function addField(msExportOrdersExcelProfileFields $field)
    {
     ...
     * @return bool|msExportOrdersExcelProfileFields
     */
    public function loadField($fieldName)
    {
        /* @var msExportOrdersExcelProfileFields $object */
        if (!$object = $this->xpdo->getObject('msExportOrdersExcelProfileFields', array(
            'profile_id' => $this->id,
            'field' => $fieldName
        ))) {
            return false;
    ...
    {
        $pid = $this->get('id');
        $table = $this->xpdo->getTableName('msExportOrdersExcelProfileFields');
     ...
     * @return bool|msExportOrdersExcelFile
     */
    public function export($classExport = null, $data = null, $config = array())
    {
        ...
        /* @var msExportOrdersExcel $msExportOrdersExcel */
        $this->msExportOrdersExcel = $this->xpdo->getService('msExportOrdersExcel', 'msExportOrdersExcel', MODX_CORE_PATH . 'components/msexportordersexcel/model/');
        $this->msExportOrdersExcel->initialize();

        /* @var msExportOrdersExcelProfileHandler $Profile */
        if ($Profile = $this->msExportOrdersExcel->newExportProfile($this, $config)) {
    ...
    public function getData()
    {
        if (is_null($this->data)) {
            ...
            $this->msExportOrdersExcel = $this->xpdo->getService('msExportOrdersExcel', 'msExportOrdersExcel', MODX_CORE_PATH . 'components/msexportordersexcel/model/');
            $this->msExportOrdersExcel->initialize();
            $query = $this->msExportOrdersExcel->query->newQuery($this->getConfig());
    ...
    public function getConfig()
    {
        ...
        foreach ($configs as $field) {
            ...
            $this->set($field, $this->xpdo->getOption('msexportordersexcel_' . $name));
        }
        return $this->toArray();
    }
    ...
    public function getFields()
    {
        ...
        if ($tmp = $this->loadFields(true)) {
            /* @var msExportOrdersExcelProfileFields $f */
            foreach ($tmp as $f) {
                $field = $f->get('field');
                $value = $f->get('value');
