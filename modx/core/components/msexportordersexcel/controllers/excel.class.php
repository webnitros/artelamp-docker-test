...
abstract class msExportOrdersExcelPhpExcelExcelController extends msExportOrdersExcelPHPExcelDefaultController
{
    ...
    protected function setDependent($owner = null)
    {
        ...
        if (!$this->dependent and !empty($this->config['dependent_profile'])) {
            ...
            if ($msExportOrdersExcelProfile = $this->modx->getObject('msExportOrdersExcelProfile', array('name' => $profile))) {
                $msExportOrdersExcelProfile->set('owner', $owner);
                if ($Profile = $this->msExportOrdersExcel->newExportProfile($msExportOrdersExcelProfile)) {
                    if ($export = $Profile->export($this->config['classExport'], true)) {
                        /* @var msExportOrdersExcelPhpExcelExcelController $export */
                        $export->dependentRows($export, $msExportOrdersExcelProfile, $this->sheet, $this->xls, $this->i);
                    }
                }
            }
     ...
     * @param msExportOrdersExcelPhpExcelExcelController $export
     ...
     * @return array
     */
    static private function dependentRows($export, $msExportOrdersExcelProfile, $sheet, $xls, $i)
    {
        $config = $msExportOrdersExcelProfile->getConfig();
        $export->sheet = $sheet;
        $export->xls = $xls;
        $export->i = $i;
