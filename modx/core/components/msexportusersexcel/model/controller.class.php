...
interface msExportUsersExcelPHPExcelInterface
{
    /**
...
abstract class msExportUsersExcelPHPExcelDefaultController implements msExportUsersExcelPHPExcelInterface
{
    /* @var \modX $modx */
    protected $modx;
    ...
    /* @var msExportUsersExcelFile|null $file */
    protected $file = null;
    ...
    /**
     * msExportUsersExcelPHPExcelDefaultController constructor.
     * @param msExportUsersExcel $msExportUsersExcel
     * @param array $config
     */
    public function __construct(msExportUsersExcel $msExportUsersExcel, array $config = array())
    {
    ...
    protected function setDefault($config = array())
    {
        $this->config = array(
            ...
            'alignmentHorizontal' => $this->modx->getOption('msexportusersexcel_alignment_horizontal', null, ''),
            'alignmentVertical' => $this->modx->getOption('msexportusersexcel_alignment_vertical', null, ''),
            ...
            'head_all' => $this->modx->getOption('msexportusersexcel_head_all', null, false),
            'head_freezepane' => $this->modx->getOption('msexportusersexcel_head_freezepane', null, false),
            'line_grouping' => $this->modx->getOption('msexportusersexcel_line_grouping', null, true),
            'line_grouping_show' => $this->modx->getOption('msexportusersexcel_line_grouping_show', null, false),
    ...
    {
        return array('msexportusersexcel:handler');
    }
    ...
    public function initialize()
    {
        if (!$this->response instanceof Response) {
            ...
            if (!($this->response instanceof Response)) {
                $this->modx->log(modX::LOG_LEVEL_ERROR, 'Could not initialize msExportUsersExcel controller handler class: "Response"');
                return false;
            }
        }
        return true;
    ...
    /**
     * Сохранит данные в объект msExportUsersExcelFile
     * @return boolean
     */
    protected function process()
    {
        ...
        if (!$filename = $this->getFilename()) {
            $this->modx->log(modX::LOG_LEVEL_ERROR, 'Could not get filename ' . print_r($this->msExportUsersExcel->profile->object->toArray(), 1));
            return false;
        }


        // Относительный путь
        $relative_path = (string)!empty($this->config['path']) ? $this->msExportUsersExcel->postfixSlash($this->config['path']) : '';
        $relative_path_file = $relative_path . $filename;


        // Абсолютынй путь
        $absolute_path = $this->msExportUsersExcel->postfixSlash($source->getBasePath()) . $relative_path;
        $absolute_path_file = $absolute_path . $filename;


        // Папка с временными данными
        $cachePath = $this->msExportUsersExcel->config['cachePath'] . $filename;
        ...
        $file = $this->msExportUsersExcel->getFile($absolute_path_file, $source);
        if (!$file instanceof msExportUsersExcelFile) {
            return false;
        }
        $this->file = $file;
     ...
     * @return mixed|msExportUsersExcelFile
     */
    public function loadFile()
    {
        if ($this->file instanceof msExportUsersExcelFile) {
            return $this->file;
        }
        return true;
    }
    ...
    protected function loadSourceInitialize()
    {
        ...
        if ($source = $this->msExportUsersExcel->loadSourceInitialize($source_id)) {
