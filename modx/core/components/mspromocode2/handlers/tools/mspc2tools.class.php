...
class mspc2Tools
{
     ...
     * @param array          $config
     */
    function __construct(msPromoCode2 &$mspc2, $config = array())
    {
        $this->mspc2 = &$mspc2;
    ...
    public function runProcessor($action = '', $data = array())
    {
        ...
        $processorsPath = !empty($this->mspc2->config['processorsPath']) ? $this->mspc2->config['processorsPath'] : MODX_CORE_PATH;

        /* @var modProcessorResponse $response */
        $response = $this->modx->runProcessor($action, $data, array('processors_path' => $processorsPath));

        return $this->mspc2->config['prepareResponse'] ? $this->prepareResponse($response) : $response;
    }
    ...
    public function prepareResponse($response)
    {
        ...
        if ($this->mspc2->config['jsonResponse'] AND is_array($output)) {
            $output = $this->modx->toJSON($output);
        } elseif (!$this->mspc2->config['jsonResponse'] AND !is_array($output)) {
    ...
    {
        if ($pdoTools = $this->mspc2->getPdoTools()) {
    ...
    public function success($message = '', $data = array(), $placeholders = array())
    {
        ...
        return $this->mspc2->config['jsonResponse'] ? $this->modx->toJSON($response) : $response;
    }
