...
if (!class_exists('GuzzleHttp\Client')) {
    include_once dirname(MODX_BASE_PATH, 1) . '/vendor/autoload.php';
}
...
class CrontabControllersync1cexportstatusorder extends modCrontabController
{
    ...
    {
        $this->statusPayment = 2; // Статус оплачен
        $this->statusSendTo1c = 3; // Статус отправки в 1с
    ...
    public function send(msOrder $order, $sync_id, $status)
    {
        ...
        if (!$Log = $order->getOne('Log', ['order_id' => $order->get('id'), 'entry' => $this->statusPayment, 'action' => 'status'])) {
            echo '<pre>';
            print_r('Не удалось получить стутс лога оплаты: '.$sync_id);
            die;
        }
        ...
        if ($sync) {
            ...
            echo '<pre>';
            print_r('Не удалось отправить статус заказа(' . $sync_id . ') PAID в 1с. Заказ '.$order->get('num').'<br>');
            print_r($response);
