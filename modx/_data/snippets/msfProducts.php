...
// Add Msf
if (empty($list)) {$list = 'mylist';}
if (empty($user_id)) {$user_id = $modx->user->id;}

$mode = $modx->getOption('msfavorites_mode', $config, 0);
if(($mode == 2) && ($user_id !== 0) || ($mode == 0)) {
	// с БД
	$where_msf['Msf.list:='] = $list;
	$leftJoin_msf = array(
		array('class' => 'msFavoritesList', 'alias' => 'Msf', 'on' => '`Msf`.`msf_id`=`msProduct`.`id` AND `Msf`.`user_id`='.$user_id.''),
	);
	$select_msf = array(
		array('msf' => $modx->getSelectColumns('msFavoritesList', 'Msf', '', array('id'), true) ),
	);
	$where = array_merge($where, $where_msf);
	$leftJoin = array_merge($leftJoin, $leftJoin_msf);
	$select = array_merge($select, $select_msf);
}
elseif(($mode == 2) && ($user_id == 0) || ($mode == 1)) $where_msf = '';
...
if (!empty($rows) && is_array($rows)) {
	...
	foreach ($rows as $k => $row) {
		...
		if(empty($where_msf)) {
			$row['msf'] = !empty($_SESSION['msfavorites'][$list]['properties'][$row['id']])
				? $modx->fromJson($_SESSION['msfavorites'][$list]['properties'][$row['id']])
				: '';
		}
